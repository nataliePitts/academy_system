...
studentRoutes.get('/', function (req, res) {
  ...
  StudentModel.find({}, function (err, students) {
    if (!err) {
      res.render('index.pug', { students, toar_type });
    } else {
...
// << -- After filter Result -- >>
studentRoutes.post('/filter', async function (req, res) {
  try {
    console.log('req', req.body);

    let { city, toar, min_avg: grade } = req.body;
    ...
    let students = await StudentModel.find(filter).lean();
